...
interface OutletStatus {
  ...
  matched_supplier: string; // exact match from OUTLET_SUPPLIERS or raw name
  has_transaction: boolean;
  total_value: number;
}

export default function SalesmanOutletCheckPage() {
  ...
  useEffect(() => {
    if (!salesmanName || !selectedYear) return;

    let cancelled = false;
    const loadData = async () => {
      setLoadingData(true);
      try {
        // Fetch all raw_data for the salesman in this year
        const { data, error } = await supabase
          .from("raw_data")
          .select("cust_ship_id, cust_ship_name, supplier_name, bln, year, gs_value")
          .ilike("ar_slsm_name", `${salesmanName.trim()}%`)
  ...
  const outletStatuses = useMemo(() => {
    ...
    for (const row of rawRows) {
      ...
      // If the row matches selected month and year, add to value map
      // Use case-insensitive bln comparison and loose year comparison to handle Supabase type variations
  ...
  const filteredOutlets = useMemo(() => {
    return outletStatuses.filter((item) => {
      // 1. Supplier filter (uses exact supplier_name, not grouped principal)
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">
          Check Outlet Transaksi
        </h1>
        <p className="text-sm text-slate-500 mt-1">
          Pantau outlet yang sudah dan belum bertransaksi untuk periode berjalan
          {salesmanName && (
            <span className="ml-1 font-semibold text-slate-700">— {salesmanName}</span>
          )}
        </p>
      </div>
